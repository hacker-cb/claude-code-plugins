...
| --- | --- | --- |
| [`hcb-dev`](plugins/hcb-dev) | `dependency-versions`, `seeding-gitignore`, `codex-review`, `multi-review`, `shipping-workflow`, `github-pr-workflow`, `cleanup-project` | Personal developer baseline — a connected pipeline: resolve dependency versions (`.github/dependabot.yml` in sync on GitHub), seed `.gitignore`, review a change locally (Codex + multi-reviewer consolidation), ship finished work to a change request, drive a GitHub PR from a finished branch to a merged PR (rename → rebase → open → CI/Copilot fix-loop → merge on your go-ahead → report), and sweep a project's git/session residue. |
