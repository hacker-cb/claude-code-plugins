...
| [`hcb-dev`](plugins/hcb-dev) | `dependency-versions`, `library-docs`, `/hcb-dev:rules` command (+ `SessionStart` / `PreToolUse` hooks) | Personal developer baseline — per-scope language (chat/plans/comments/docs/issues) injected each session via a configurable hook; sync canonical cross-project rules into `.claude/rules/hcb/` with `/hcb-dev:rules` (guarded against hand-edits, drift-checked each session); resolve dependency versions from the registry (keeping `.github/dependabot.yml` in sync on GitHub repos); and pull current library docs from Context7. |
