...
| --- | --- | --- |
| [`hcb-dev`](plugins/hcb-dev) | `implementation-workflow`, `issue-tracking`, `dependency-versions`, `seeding-gitignore`, `codex-review`, `multi-review`, `shipping-workflow`, `github-pr-workflow`, `session-dispatch`, `session-handoff`, `git-cleanup` | Personal developer baseline — a connected pipeline: turn tasks or issues into independently reviewable slices behind one planning gate; keep deferred work in the forge's issue tracker, classified against what the repository actually defines; resolve dependency versions (`.github/dependabot.yml` in sync on GitHub); seed `.gitignore`; review a change locally (Codex + multi-reviewer consolidation); complete each slice by mode — merged locally into its parent, or an open change request (PR/MR), with the full drive from a finished branch to a merged PR on GitHub (rename → rebase → open → CI/Copilot fix-loop → merge on your go-ahead → report); hand work across sessions as a pasteable prompt in either direction; and sweep a repository's git residue (merged branches, stale worktrees). |
| [`hcb-taobao`](plugins/hcb-taobao) | `product-search`, `item-details`, `cart-and-orders`, `seller-chat` | Shopping and sourcing on Taobao and Tmall through the **Taobao desktop client** running on your own machine — keyword research and supplier shortlists, a listing's real price, SKUs, specs, reviews and Q&A, resolving a variant into the cart, your orders and browsing history, and talking to sellers in Chinese. A bundled companion drives the client's local CLI: it paces requests around the anti-bot throttle, tells a genuinely empty result from a silent block, and catches the failures the vendor CLI reports as success. Stops at the cart — there is no checkout, and paying stays with you. Requires the client installed, signed in, and with 启用 AI 代理 switched on; every call goes through Alibaba's telemetry (see the [plugin README](plugins/hcb-taobao/README.md)). |
