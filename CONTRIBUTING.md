...
2. Optional supporting files live next to `SKILL.md`: `references/*.md`, `scripts/*`, etc. Reference them from `SKILL.md` so Claude knows when to load them.
   ...
   **A bundled script is named by how the file reaches Claude.** A `SKILL.md` writes `"${CLAUDE_PLUGIN_ROOT}/scripts/x.mjs"`, which Claude Code substitutes as it loads the skill. A `references/*.md` arrives through `Read`, where that placeholder would stay literal text and the command would run from `/`, so it writes `<plugin root>` — and every skill whose links reach such a file carries the binding line CLAUDE.md gives, last in its opening paragraph. `scripts/validate.sh` checks three things here: no substitution placeholder in a by-path file, `<plugin root>` in a `SKILL.md` only on that line, and the binding present exactly where a skill reaches a file that uses it.
