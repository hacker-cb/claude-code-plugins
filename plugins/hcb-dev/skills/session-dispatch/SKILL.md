...
is something to build; a tracker-only order names what runs instead>, in full:
its local review across every reviewer, and the subagents its skills ask for,
are part of this order. <Plus <domain methodology> — mandatory.> <checks> must
pass.
