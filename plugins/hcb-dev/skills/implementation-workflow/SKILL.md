...
`hcb-dev:status` reads one — a listing by branch name answers for forks too, so identity decides. The slice in flight stands where its branch does — cut, developed,
