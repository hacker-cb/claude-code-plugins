...
beside it. The verdict's `delta` is what the pass acts on — except for an issue labelled `epic` or
`wave`, which is structure and takes no verdict ([`../../../references/epic-structure.md`](../../../references/epic-structure.md)):
