...
**The re-verification list is the union** of `entered`, `edited`, `linked` and
`events` — every verdict it names is re-derived, every other one carried from
the ledger.
...
names none says so, and the next moment is then taken from before that read
began.
...
gh api graphql -F owner='{owner}' -F name='{repo}' -f query='query($owner: String!, $name: String!) { repository(owner: $owner, name: $name) { pullRequests(states: OPEN, first: 100) { totalCount nodes { number baseRefName changedFiles files(first: 100) { nodes { path } } } } } }' \
  --jq '.data.repository.pullRequests | {open: .totalCount, read: (.nodes | length)}, (.nodes[] | {n: .number, base: .baseRefName, short: (.changedFiles > (.files.nodes | length)), files: [.files.nodes[].path]})'
...
glab api graphql -f query='query { project(fullPath: "<path>") { mergeRequests(state: opened, first: 100) { count nodes { iid targetBranch diffStatsSummary { fileCount } diffStats { path } } } } }'
```
