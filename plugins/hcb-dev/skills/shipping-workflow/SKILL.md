...
3. **Land the branch on `parent`, before anyone reads it** — on the **ref** step 1
   refreshed, `<remote>/<parent>`, or the local branch where no remote carries one.
   The bare name is a destination, not a tip (`slice-completion.md`): land on it and
...
5. **Apply the fixes, then commit them** — that skill reports, it does not fix.
   ...
   **A fix answers its finding and stops.** Hardening that no finding asked for, a
...
before completing**: a report whose `## Review coverage` names the gap and whose blocking ask
passes on whatever the review says would close it
([`../../references/report-blocks.md`](../../references/report-blocks.md)); complete only once
the user says to. This holds in **both modes** — a *local* merge with a reviewer
