...
| a **new issue** born mid-epic | classify it here (`hcb-dev:issue-tracking`) — a worker's proposed milestone or label is a claim to verify, not a decision to adopt | — |
| a batch reports **ready**, in whichever words its order gave it for the mode it runs in (`hcb-dev:wave-dispatch`) | speak the queue to the batches it moves: the go to the batch whose slot arrived, BEHIND — with the seam owing a rebase — to the batch a landing displaced. A queue whose go never reaches its batch is a deadlock, not an order. A batch holding an `ask` reaches the user through this session, its recommendation first, and the answer travels back down as that batch's go. Where the epic stands at `ask` and the plan fixes a landing order, the order is held here: a later batch's question waits until the earlier one has landed, rather than reaching the user beside it | merge queue and gates |
| a **landing** — by its batch, by another session, or by the user | the merge itself is never this session's to take: the batch takes it, and everything its own completion owes behind it. Check the landing against the gates: a wave whose gate just cleared goes back to Launching, and so does a landing that opens the next step of a staged wave already running. A landing that happened without this session's go — a request merged from outside the queue — reaches its batch before anything else is sent, because the tail behind that merge is still its own to run | the landing with whoever took it, and what its tail left standing; the gates |
| a landing **freed ground**, or the user asks what else can run beside what is running, what blocks, or what to take next | recompute, never recall (`hcb-dev:wave-refresh`): it reads the delta since the pin it last recorded rather than the backlog again, and what it frees goes back to Launching behind the user's word on its layout | what that pass writes |
| a **wave** opens or closes | advance the epic's human half too — the wave table in its body, not only the ledger comment | the wave |
