...
and hand the user its closing table. **The preconditions that table carries go up with it** — a batch
...
- **Free capacity is recomputed, not recalled** (`hcb-dev:wave-refresh`) —
  ...
  what is running, what blocks, or what to take next. The pass reads the delta
  since the pin it last recorded rather than the backlog again, and what it
  frees goes back to Launching.
