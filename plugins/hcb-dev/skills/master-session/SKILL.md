...
| **candidates** for the tracker — in a status the moment one earns it, or with a return | record each in the ledger as it arrives, whenever it arrives, then rule them cold and together — never on the one that carried them: they accumulate and are decided in one pass at the close of the round, read against each other and against what the backlog already holds (`findings.md`), each prepared as that file asks before the pass goes up rather than after the user sends it back to be re-checked; each ends in one of that file's outcomes, and a candidate ruled **DROP** is answered with the reason. The ruling carries its own authorization (`Who decides`), and an outcome that section leaves with the user goes up first, with a recommendation, while one landing on ground this epic does not hold — a **HAND OVER** — goes to whoever holds that ground rather than being opened here. What the authority does cover is opened here — a batch accepted on its return is free before this pass runs, so a worker files its own only where the pass ruled its candidates before it was released | the candidates as they arrive; the rulings |
...
Three things stay the user's, and go up with this session's recommendation first
...
The click that starts a chip and an approval on the forge are the user's by
construction: this session asks for them and never works around them.
...
where the last wave has closed — rather than riding every line — and a tracker write the next wave stands on that `Who decides` leaves with
the user holds work whatever its rating, so it stands in that group until it is
answered or deferred. The block carrying them is the ledger's expectations addressed to the user
