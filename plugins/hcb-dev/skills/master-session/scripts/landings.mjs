...
// about, and consecutive commits of one request, or of no request, are one landing.
//
// Usage: node landings.mjs --since <commit> --to <commit> --base <branch>
//          [--remote <name> [--forge gh|glab] | --no-forge] [--repo-dir <path>]
//
//   --since, --to  the range: `since.sha` and `since.to` of `master-tree.mjs --since`
//   --base         the branch they landed on, as the forge names it
//   --remote       the remote carrying that branch: its url names the forge host and the
//                  repository asked — without it, whichever the checkout's CLI defaults to
...
import { dirOk, forgeFor, projectPathOk, refNameOk, remoteOfUrl, requestsOf, runner, writeAll } from '../../../scripts/lib/forge.mjs';
...
const opts = { since: null, to: null, base: null, forge: true, cli: null, remote: null, repoDir: null };
const FLAGS = { '--since': 'since', '--to': 'to', '--base': 'base', '--forge': 'cli', '--remote': 'remote',
  '--repo-dir': 'repoDir' };
...
for (const f of ['since', 'to', 'base']) if (!opts[f]) die(`--${f} is required`);
for (const [flag, key] of Object.entries(FLAGS)) {
  if (['since', 'to', 'base', 'remote'].includes(key) && opts[key] !== null && !refNameOk(opts[key])) {
    die(`${flag} '${opts[key]}' is not a name this can read`);
  }
}
if (opts.cli && !['gh', 'glab'].includes(opts.cli)) die(`--forge '${opts.cli}' is not gh or glab`);
if (!opts.forge && (opts.remote || opts.cli)) die('--no-forge asks no forge: --remote and --forge are for one that does');
...
const MAX = 100;
// Whose requests are asked about: the remote's own repository on its own host, where one is named.
let where = { host: null, path: null };
...
const answer = {
  ...
  forge: { asked: opts.forge, cli: null, host: null, repo: null, reason: null },
  ...
  //   request · a change request merged into the base
  //   push    · commits no merged request into the base carries
  //   commit  · one commit, with --no-forge
  //   unsure  · several merged requests carry it and none by its merge commit: `requests`
  //   unread  · the forge did not answer about it
  landings: [],
...
if (opts.forge && answer.commits > MAX) {
  answer.reason = `${answer.commits} commits — past the ${MAX} this asks the forge about one by one`;
  finish();
}
const l = git(['log', '--no-show-signature', '--first-parent', '--reverse', '--format=%H %s', ...range]);
if (!l.ok) { answer.read = false; refuse(`could not list the commits between them (${why(l)})`); }
const lines = l.out.split('\n').filter(Boolean);

// What each commit is, one at a time; runs of one request, or of none, are joined below.
const keyed = lines.map((line) => ({ sha: line.split(' ')[0], line, kind: 'commit' }));
if (opts.forge) {
  if (opts.remote) {
    const u = git(['remote', 'get-url', opts.remote]);
    const r = u.ok ? remoteOfUrl(u.out.split('\n')[0]) : null;
    if (r && projectPathOk(r.path)) where = r;
    else answer.forge.reason = u.ok ? `${opts.remote}'s url names no forge repository` : `could not read ${opts.remote}'s url (${why(u)})`;
  }
  const found = answer.forge.reason ? null : forgeFor(cwd, opts.cli, where.path, where.host);
  Object.assign(answer.forge, { cli: found?.cli ?? null, host: where.host, repo: where.path });
  if (found?.reason) answer.forge.reason = found.reason;
  for (const k of keyed) {
    // One refusal is the forge's answer for the rest of the range: asking on only multiplies it.
    if (!found?.reader || answer.forge.reason) { k.kind = 'unread'; continue; }
    const got = requestsOf(found, cwd, where.path, k.sha);
    if (got.error) { answer.forge.reason = got.error; k.kind = 'unread'; continue; }
    const merged = got.rows.filter((q) => q.state === 'merged' && q.base === opts.base && Number.isInteger(q.number));
