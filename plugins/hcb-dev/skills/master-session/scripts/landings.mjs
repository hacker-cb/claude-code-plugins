...
//
// Usage: node landings.mjs --since <commit> --to <commit> --base <branch> [--no-forge]
//          [--forge gh|glab] [--repo <owner/name>] [--repo-dir <path>]
//
//   --since, --to  the range: `since.sha` and `since.to` of `master-tree.mjs --since`
//   --base         the branch they landed on, as the forge names it
//   --no-forge     every commit a landing of its own — a parent the epic completes into
//                  locally, where no forge merges anything
//
...
import { dirOk, forgeFor, refNameOk, refOk, repoOk, runner, writeAll } from '../../../scripts/lib/forge.mjs';

const USAGE = 'usage: node landings.mjs --since <commit> --to <commit> --base <branch> [--no-forge]'
  + ' [--forge gh|glab] [--repo <owner/name>] [--repo-dir <path>]\n';
const die = (m) => { writeAll(2, `landings: ${m}\n${USAGE}`); process.exit(2); };

const argv = process.argv.slice(2);
const opts = { since: null, to: null, base: null, forge: true, cli: null, repo: null, repoDir: null };
const FLAGS = { '--since': 'since', '--to': 'to', '--base': 'base', '--forge': 'cli', '--repo': 'repo',
  '--repo-dir': 'repoDir' };
...
const answer = {
  ...
  commits: null,
  // `asked: false` with --no-forge. `reason` where a forge was wanted and none could be asked:
  // every landing is then `unread`, never a push.
...
const keyed = [];
if (!opts.forge) {
  for (const line of lines) keyed.push({ kind: 'commit', line });
} else {
  const found = forgeFor(cwd, opts.cli, opts.repo);
  answer.forge.cli = found.cli;
  answer.forge.reason = found.reason;
  const forge = found.reader ? runner(cwd, found.cli) : null;
  for (const line of lines) {
    const sha = line.split(' ')[0];
    if (!forge || !refOk(sha)) { keyed.push({ kind: 'unread', line }); continue; }
    const r = forge(['api', ...found.hostArgs, '--paginate', found.reader.path(opts.repo, sha)]);
    const rows = r.ok ? found.reader.read(r.out) : null;
    if (rows === null) {
      if (!answer.forge.reason) answer.forge.reason = r.ok ? 'the answer was not JSON' : why(r);
      keyed.push({ kind: 'unread', line });
      continue;
    }
    const merged = rows.filter((q) => q && typeof q === 'object').map(found.reader.row)
      .filter((q) => q.state === 'merged' && q.base === opts.base && Number.isInteger(q.number));
    // A request's own merge commit settles it; otherwise one request, or none, is the answer.
    const own = merged.filter((q) => q.mergeCommit && q.mergeCommit.toLowerCase() === sha.toLowerCase());
    const pick = own.length === 1 ? own : merged;
    if (pick.length === 1) keyed.push({ kind: 'request', number: pick[0].number, line });
    else if (!pick.length) keyed.push({ kind: 'push', line });
    else keyed.push({ kind: 'unsure', requests: pick.map((q) => q.number), line });
  }
}
...
for (const k of keyed) {
  ...
  const tip = k.line.split(' ')[0];
  if (joins) { last.commits.push(k.line); last.tip = tip; continue; }
  answer.landings.push({ kind: k.kind, ...(k.number ? { number: k.number } : {}),
    ...(k.requests ? { requests: k.requests } : {}), commits: [k.line], tip });
}
