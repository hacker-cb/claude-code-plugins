...
### Gates are a floor, never a ceiling
...
not "the work is ready." Your own bar applies on top of whatever the repo enforces:
CI actually green, Copilot's findings actually addressed, branch current with base.
Where the two diverge, the stricter wins.

That is enough to run the loop. What the typed rules mean once you have read them,
how to tell an advisory or bypassable gate from a real one, and how to establish
that a repo has **none** (an empty `/rulesets` does not prove it — classic branch
protection is invisible there) are lookup:
[`references/merge-gates.md`](references/merge-gates.md).

## Step 1 — Publish the branch under the right name

Choose the name per [`../../references/branch-naming.md`](../../references/branch-naming.md)
and rename with `git branch -m`; that file also names the two cases where the
rename is off the table. This is the **last** point a rename is possible at all —
once the PR is open, renaming means deleting its head ref, and that closes the PR.

Four things are local to this step, and nothing else here is:

1. **Resolve the push remote before renaming.** It is git's own push routing —
   `branch.<name>.pushRemote`, then `remote.pushDefault`, then `origin`, then a
   lone remote whatever its name — and **never** `@{upstream}`, which in a fork is
   the canonical repo. Several remotes and none preferred: stop and ask rather than
   publish in someone else's repository. Resolve *first*, because that stop exits,
   and exiting after `git branch -m` leaves a branch renamed locally with nothing
   pushed. See [`../../references/base-resolution.md`](../../references/base-resolution.md),
   "Pushing is a different question", and route the push through its
   non-interactive guard.
2. **The push is unconditional.** The rename may be a no-op — `hcb-dev:shipping-workflow`
   step 0 usually normalized the name already — but this is the only place the
   branch reaches the remote, and Step 2's `--force-with-lease` dies on "no upstream
   branch" while Step 3's `--head` finds no ref.
3. **Delete the old remote ref only on a positive answer.** This is the one
   irreversible action in the step: deleting the head ref of an open PR closes it
   and takes its review threads with it. Require the forge to *say* no PR points at
   the branch — an error, an empty result from a misconfigured `gh`, or no CLI at
   all all look identical and none of them is a yes. A stale remote branch costs
   nothing.
...
Rebase onto the latest base — cleaner history, and it plays well with squash.

- **The base's remote is not the push remote.** In a fork the base lives upstream
  while you push to your own copy. Probe each remote for one that already carries
  `<base>`, fall back to preference (`upstream`, `origin`, a lone remote), and stop
  on a genuine ambiguity.
- **Check the fetch, not the ref.** The ref exists precisely because a stale copy is
  already there, so an existence test passes against a week-old base: a failed fetch
  (expired credential, VPN down) would rebase onto it, report "up to date", and let
  GitHub report `BEHIND` at merge time. "Did not update" is the only failure this
  step has.
- **Resolve trivial conflicts; stop on one that needs a decision.**
- **A shared branch is not rebased** — others have commits or open PRs on it. Merge
  base into the branch instead and say why.
- **Under a strict required-checks policy, being current is itself a gate.** Whenever
  GitHub reports `BEHIND` — including right before merge — re-sync;
  `gh pr update-branch <pr>` does it server-side.

After a successful rebase, push with `--force-with-lease`, never plain `--force`.
...
Critical/Important findings resolved (see *Gates are a floor, never a ceiling*). Up to ~5 iterations, then escalate. Gates decide *permission* to
...
integration PR**), always **filtered to the repo's allowed merge methods** (from
