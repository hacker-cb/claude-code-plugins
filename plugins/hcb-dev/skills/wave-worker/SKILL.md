...
- **The order's facts are re-verified before anything rests on them**: read
  ...
  list of the order's facts to re-check — refreshing a base here where the
  named workflow has no refresh of its own — and a premise that falls is named
  to the master the moment it falls, never saved for the return.
