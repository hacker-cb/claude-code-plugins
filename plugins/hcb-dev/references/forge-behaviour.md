...
| `glab api` input | **list fields passed as `-f 'x[]=v'` are dropped without a word**; and `--input` sends no `Content-Type`, which GitLab refuses with `415` ([gitlab-org/cli#8517](https://gitlab.com/gitlab-org/cli/-/issues/8517)) | a JSON body through `--input` with `-H 'Content-Type: application/json'` |
