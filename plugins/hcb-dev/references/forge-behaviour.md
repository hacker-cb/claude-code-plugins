...
|---|---|---|
| a comment body's maximum | **a `422` whose text names the wrong number in the wrong unit.** The cap is **262 144 bytes of UTF-8**, measured on both sides of the boundary for a comment's REST create and edit alike: 262 144 bytes stored, 262 145 refused with `body is too long (maximum is 65536 characters)` — and a Cyrillic body refused at the same byte count while 131 086 characters long. GraphQL's `updateIssueComment` answers with the same text. A refusal without that status and text is transport, not the cap: an empty reply or a response that does not parse refused 22 of 33 writes of one body under the cap, stored between them — one such refusal came from a write that had landed — and a secondary-limit `403` arrives the same way | measure **UTF-8 bytes** against the cap before writing, and archive ahead of it. Answer a `422` naming the length by archiving; answer any other refusal by reading the body back, and writing again only if it did not land. GitLab's own limit is unmeasured |
