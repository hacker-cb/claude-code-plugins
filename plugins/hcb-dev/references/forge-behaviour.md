...
|---|---|---|
| a comment body's maximum | **a refusal saying the body is too long, and naming the wrong number in the wrong unit.** The cap is **262 144 bytes of UTF-8**, measured on both sides of the boundary for a comment's REST create and edit alike: 262 144 bytes stored, 262 145 refused with `422` and `body is too long (maximum is 65536 characters)` — and a Cyrillic body refused at the same byte count while 131 086 characters long. **The status belongs to REST alone**: GraphQL's `updateIssueComment` refuses the same 262 145 bytes under `HTTP 200`, as an `UNPROCESSABLE` entry in `errors` carrying `Body is too long (maximum is 65536 characters)`. A refusal saying nothing about the length is transport, not the cap: an empty reply or a response that does not parse refused 22 of 33 writes of one body under the cap, stored between them — one such refusal came from a write that had landed — and a secondary-limit `403` arrives the same way | measure **UTF-8 bytes** against the cap before writing, and archive ahead of it. Answer a refusal whose message says the body is too long by archiving, whichever API carried it; answer any other refusal by reading the body back, and writing again only if it did not land. GitLab's own limit is unmeasured |
