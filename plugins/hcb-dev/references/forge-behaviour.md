...
| a check-run's `name` | **not unique on a commit.** Measured: one merge commit carried `audit-full`, `audit-health`, `audit-runtime` and `dependabot-health` three times each, one per matrix leg — keyed by name, twenty-two runs read as eight | key by `id`; report by name | — |
| the set of checks on a merge commit | **not the set the head carried.** Measured on one pull request: the head ran 9, the merge commit 22, the extra fourteen being audit and dependabot workflows that never run for a pull request | read the merge commit's own feeds after the merge lands | a base whose workflows all run on pull requests too |
