...
- **The base pin** — the base as `<remote>/<branch>@<sha>`, the remote-tracking
  ...
  tracker's alone has no delta of this kind. An issue among the facts is
  re-verified either way, and against the code rather than the tracker
  ([`issue-currency.md`](issue-currency.md)): the pin dates what the tree made
  true, never what the body claims about it.
