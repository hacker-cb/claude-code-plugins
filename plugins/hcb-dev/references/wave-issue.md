...
wave's table instead; one no wave has taken yet stays under the epic. An issue a wave takes loses
its parked reason where the approved plan names it ([`label-lifecycle.md`](label-lifecycle.md)).
Read the edges and the labels back once they are written.
