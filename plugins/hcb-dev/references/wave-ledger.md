...
1. **Header** — the epic, the master's name — rewritten whenever it changes —
   ...
   survey's — together with the moment it read the tracker at, the two halves
   the next refresh takes its delta from, since an issue closes without a
   commit (`hcb-dev:wave-refresh`),
