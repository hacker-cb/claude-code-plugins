...
- **Dependency edges.** What blocks what, read from the issues and the tree —
  not assumed from titles. An edge the plan stands on that the tracker lacks is
  proposed as a link ([`issue-links.md`](issue-links.md)). A chain with one unblocked vertex is one batch in
