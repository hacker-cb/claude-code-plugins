...
the shape and at the moment [`session-naming.md`](session-naming.md) gives. A
chip-started session is born titled with its chip's title, so a dispatcher that
put the identifier into the chip title has titled the receiver already. Where no
title tool exists, the worktree name is the stable half of the live name — coin
it by the same reference.
