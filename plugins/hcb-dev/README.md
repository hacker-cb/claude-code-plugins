...
- **`master-session`** — `/hcb-dev:master-session`
  ...
  the landing order calls for one, is its whole part in a landing. Recovers after
  a restart from the ledger before the live registry. It does not build batches
  itself.
